...
export const updateBlog = TryCatch(async (req: AuthenticatedRequest, res) => {
  ...
  // Fixed: Using correct field variables instead of 'title' for everything
...
image tags, line breaks, and structural tags exactly as they are. Return the full corrected HTML string as output.`;
  ...
  async function main() {
    const response = await ai.models.generateContent({
      model: "gemini-2.0-flash", // Use the same model as others for consistency
      contents: fullPrompt,
    });
    ...
    if (!rawtext) {
      res.status(400).json({
        message: "Failed to generate corrected content",
      });
      return;
    }
